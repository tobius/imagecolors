...
// test images
var images = [ 'bluejay', 'borealis', 'country', 'crescent', 'fish', 'flower', 'googleglass', 'parrot', 'petals', 'puppy', 'soccer', 'tigers' ];
...
images.forEach(function(image){
    ...
    imagecolors.extract(__dirname + '/photos/' + image + '.jpg', 5, function(err, colors){
        ...
        imagecolors.convert(colors, __dirname + '/palette.json', function(err, colors){
            ...
            if (_.keys(entries).length === images.length && !done){
                ...
                fs.writeFileSync(__dirname + '/convert.html', head + _.values(sortByKey(entries)).join('') + foot, 'utf8');
