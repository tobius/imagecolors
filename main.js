...
module.exports = {
	...
	convertPathToImage: function(path, callback) {
		...
		fs.lstat(path, function(err, stats) {
			...
			return convert('Unable to quantify image path');
		});
	...
	buildColorProfile: function(hex) {
		...
		luminance = Math.round(
			parseFloat(
				(rgb.r * 0.2126 + rgb.g * 0.7152 + rgb.b * 0.0722) * (1 / 255)
			).toFixed(2) * 100
		);
	...
	mergeSimilarColors: function(colors) {
		...
		var i, j, delegate, toleranceMax, toleranceThreshold,
			pixels, absorbedColors, dominantColors,
			distance, color1, color2, keepDominantColor;
